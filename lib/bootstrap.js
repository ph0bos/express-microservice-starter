...
module.exports = function (app, options) {
  ...
  if (options.consul) {
    consularis.registerService(options.serviceName)
      .then((data) => {
        if (options.debug) {
          log.info(data, 'microservice registered with consul');
        }
        console.log(data);
        app.locals.microservice = data;
        app.emit('service:registered', data);
    ...
    process.on('SIGINT', () => {
      ...
      console.log(app.locals.microservice);

      consularis.deregisterService(app.locals.microservice.id)
        .then(() => {
